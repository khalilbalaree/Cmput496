...
class SimpleGoBoard(object):
    ...
    def simulate(self):
        ...
        if not self.check_game_end_gomoku()[0]:
            ...
            for i in range(len(allMoves)):
                thisMove = allMoves[i]
                self.play_move_gomoku(thisMove, self.current_player)
    ...
    def resetToMoveNumber(self, num):
        numUndos = self.moveNumber() - num
        assert numUndos >= 0
        for _ in range(numUndos):
            self.undo_move()
        assert self.moveNumber() == num

    def getSingleMoveByPolicy(self):
        moveFound, _ = rules(self.current_player, self.board, self.NS).policy_type_search_move()
        if moveFound != None:
            return moveFound[0]
